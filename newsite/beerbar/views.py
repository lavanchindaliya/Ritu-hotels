...
from .models import Restaurant, Task,  Club, Banquet,  Meeting, Room,  Order
...
def add_to_cart(request, name, price, category):
    if request.user.is_authenticated:
            if request.method == 'POST':
                ...
                pi.save()
            if category == 'room':
                return HttpResponseRedirect('/products_room')
